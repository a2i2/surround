...
def task_status():
    ...
    return {"actions": ["%s -m %s status=1" % (sys.executable, PACKAGE_PATH)]}


def task_build():
    """Build the Docker image for the current project"""
    return {"actions": ["docker build --tag=%s ." % IMAGE], "params": PARAMS}
...
def task_dev():
    ...
    return {"actions": [" ".join(cmd)], "params": PARAMS}
...
def task_prod():
    ...
    return {
        "actions": [
            "docker run %s python3 -m %s %s" % (IMAGE, PACKAGE_PATH, "%(args)s")
        ],
        "task_dep": ["build"],
        "params": PARAMS,
    }
...
def task_train_local():
    ...
    cmd = [sys.executable, "-m %s" % PACKAGE_PATH, "mode=train", "%(args)s"]

    return {"basename": "trainLocal", "actions": [" ".join(cmd)], "params": PARAMS}


def task_batch_local():
    """Run batch mode locally"""
    cmd = [sys.executable, "-m %s" % PACKAGE_PATH, "mode=batch", "%(args)s"]

    return {"basename": "batchLocal", "actions": [" ".join(cmd)], "params": PARAMS}
