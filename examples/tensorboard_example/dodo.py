...
def task_train():
    ...
    return {
        'actions': ["docker run --volume \"%s\":/app/output --volume \"%s\":/app/data %s python3 -m tensorboard_example --mode train" % (output_path, data_path, IMAGE)]
    }
...
def task_batch():
    ...
    return {
        'actions': ["docker run --volume \"%s\":/app/output --volume \"%s\":/app/data %s python3 -m tensorboard_example --mode batch" % (output_path, data_path, IMAGE)]
    }
...
def task_train_local():
    ...
    return {
        'basename': 'trainLocal',
        'actions': ["%s -m tensorboard_example --mode train" % sys.executable]
    }
...
def task_batch_local():
    ...
    return {
        'basename': 'batchLocal',
        'actions': ["%s -m tensorboard_example --mode batch" % sys.executable]
    }
