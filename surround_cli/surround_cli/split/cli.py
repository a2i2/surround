...
def get_split_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Randomly assign data to test, training, and validate sets"
    )
    ...
    group.add_argument(
        "-t",
        "--text-file",
        help="Split text file into train/test/validate sets",
        type=lambda x: is_valid_file(parser, x),
    )
    group.add_argument(
        "-d",
        "--directory",
        help="Split directory into train/test/validate sets",
        type=lambda x: is_valid_dir(parser, x),
    )
    group.add_argument(
        "-r",
        "--reset",
        help="Path to directory containing train/test/validate folders to reset",
        type=lambda x: is_valid_dir(parser, x),
    )
    ...
    parser.add_argument(
        "-e",
        "--extension",
        help="File extension of the files to process (default: *)",
        type=str,
        default="*",
    )
    parser.add_argument(
        ...
        "--train",
        type=int,
        help="Percentage of files for training (default: 80%%)",
        default=80,
    )
    parser.add_argument(
        "-te",
        "--test",
        type=int,
        help="Percentage of files for test (default: 10%%)",
        default=10,
    )
    parser.add_argument(
        "-va",
        "--validate",
        type=int,
        help="Percentage of files for validate (default: 10%%)",
        default=10,
    )
    parser.add_argument(
        "-nv",
        "--no-validate",
        action="store_true",
        help="Don't produce a validation set when splitting",
    )
    parser.add_argument(
        "-ns",
        "--no-shuffle",
        action="store_true",
        help="Don't randomise when splitting data",
    )
    parser.add_argument(
        "-nh",
        "--no-header",
        action="store_true",
        help="Use this flag when the text file has no headers",
    )
...
def validate_args(args):
    ...
    if args.reset:
        dirs = [
            path
            for path in os.listdir(args.reset)
            if os.path.isdir(os.path.join(args.reset, path))
        ]
        expected = ["train", "test"]

        if not all([exp in dirs for exp in expected]):
            print(
                "Cannot reset this folder since there are no test/train/validate folders!"
            )
            return False
