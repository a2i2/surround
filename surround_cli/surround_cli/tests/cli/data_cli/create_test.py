...
class CreateDataContainerTest(unittest.TestCase):
    ...
    def test_happy_path(self):
        ...
        process = subprocess.run(
            ["surround", "data", "create", "-d", "temp", "-o", "temp.data.zip"],
            input=std_input,
            encoding="ascii",
            check=True,
        )
        ...
        self.assertEqual(
            metadata["manifests"][0]["description"], "Test group description"
        )
        self.assertIn("image/png", metadata["manifests"][0]["formats"])
        self.assertNotIn("image/jpeg", metadata["manifests"][0]["formats"])

        self.assertTrue(
            all([container.file_exists("test_group/%i.png" % i) for i in range(100)])
        )
        self.assertTrue(
            all([container.file_exists("derp_%i.jpg" % i) for i in range(20)])
        )
