...
class TestDataContainer(unittest.TestCase):
    ...
    def test_import_file(self):
        ...
        csv_in_meta = (
            "application/vnd.ms-excel" in container.metadata["summary"]["formats"]
        )
        ...
        with zipfile.ZipFile("test-container.data.zip", "r") as f:
            ...
            with f.open("test_file.csv") as ff:
                self.assertEqual(
                    "ground_truth,predict_value", ff.readline().decode("utf-8").rstrip()
                )
    ...
    def test_extract_file(self):
        ...
        with open("test_file.csv", "rb") as f:
            self.assertEqual(
                "ground_truth,predict_value", f.readline().decode("utf-8").rstrip()
            )
